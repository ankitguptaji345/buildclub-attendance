// Wake Render's free-tier server so the unlock click doesn't fail
...
function updatePoseUI() {
  if (poseIndex >= POSES.length) {
    if (poseInstruction) poseInstruction.textContent = '✅ All 5 angles captured! Click "Save Registration" to finish.';
    ...
    if (captureBtn) captureBtn.textContent = `📸 Capture This Angle (${poseIndex + 1}/${POSES.length})`;
  }
  renderPoseDots();
}
...
captureBtn.addEventListener('click', async () => {
  ...
  if (poseIndex < POSES.length) {
    if (!latestDetection) {
      showStatus('⚠️ No face detected yet. Center your face in the frame and try again.', 'err');
      return;
    }
  ...
  try {
    const res = await fetch('/api/members/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ buildClubId, name, descriptors: capturedDescriptors, adminPassword })
    });
    const data = await res.json();
    if (res.ok) {
...
// If this tab already unlocked once, hint at it - but still require the
// password again for safety (we never store the password itself).
if (sessionStorage.getItem('bc_admin_ok') === '1') {
  showLockStatus('ℹ️ You unlocked this tab before — type the password again to continue.', 'info');
}
